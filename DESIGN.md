...
— every count the phases carry stands at none where the pipeline ends, and what is left is what `OWED` names: the
invariants no phase has taken on, each carrying a test of its own so its count is read here beside them rather than
written into a document by hand and left to go stale.
