...
"""Translate the vendored yaml-grammar into the typed IR (`grammar.py`).

Faithful 1:1: every yaml-grammar operator maps to exactly one IR node, with no flattening or normalization, so the
round-trip through `grammar2spec.py` stays exact. Anything unexpected raises loudly rather than being dropped silently.

Usage: `python3 generator/spec2grammar.py [spec.yaml] > grammar.py`
"""
...
DEFAULT_SPEC = "third_party/yaml-grammar/yaml-spec-1.2.yaml"
...
def load(source=DEFAULT_SPEC):
    """Load and translate the vendored grammar."""
    with open(source) as handle:
        return translate(yaml.safe_load(handle))


def main():
    productions = load(sys.argv[1] if len(sys.argv) > 1 else DEFAULT_SPEC)
    out = sys.stdout
    out.write("# Generated by spec2grammar.py from the vendored yaml-grammar. Do not edit.\n")
