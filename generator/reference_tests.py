...
"""The vendored reference tests, as the parser generator sees them.

`third_party/yamlreference/tests/` holds the static differential oracle: for each
`<production>[.n=N][.c=C][.t=T].<case>` there is an `.input` YAML fragment and an `.output` YEAST token stream the
reference parser emits for it (an `.invalid`
case's output ends in an `Error` token). The filename says which production to run and with which parameters — this
module decodes that convention, mirroring the reference's own test runner, and pairs each input with its expected
output.

Not every fixture maps onto libyeast's grammar: some exercise the reference parser's internal helper productions
(`detect-*-indentation`, `count-spaces`, …) or its private parameterization, which are not the official grammar and
which libyeast does not replicate. `is_runnable` tells the two apart, so the harness runs what aligns and skips the rest
without pretending the grammar is wrong.
"""
...
TESTS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "third_party", "yamlreference", "tests"
)
