...
Faithful and exact. A yaml-grammar operator maps to a single IR node. Flattening and normalization stay out of this. The
round-trip through `ir2annotated.py` stays exact. Anything unexpected raises loudly rather than dropping silently.
...
# the build knows, and no further. That keeps a fixture from naming a policy outside them.
CONTEXTS = ("block-in", "block-out", "block-key", "flow-in", "flow-out", "flow-key")
CHOMPINGS = ("strip", "clip", "keep")  # the values `t` may take. A block scalar treats its trailing breaks by that.
...
_HEX = re.compile(r"^x[0-9A-Fa-f]+$")  # a codepoint written `xHH`. That is the other way to write a character.
_REP = re.compile(r"^\(\{(.+)\}\)$")  # `({2})` and `({n})`.
...
def _is_char(x: object) -> TypeGuard[str]:
    """Whether `x` is a single character or an `xHH` hex codepoint. Those are what `char` can translate."""
...
def written(source: str = DEFAULT_GRAMMAR) -> dict[str, object]:
    """
    The grammar at `source` as the file writes it, as `{name: what it says}`. For what a caller asks of the source
    rather than of the IR.

    The file names a production twice. A `:N` key gives the number and the name it goes under, and that name keys what
    the production says. So the loop's key is the number and its value the name. `translate` reads it that way too.
    """
    with open(source, encoding="utf-8") as handle:
