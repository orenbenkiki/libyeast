...
def denote(grammar, node, seen=()):
    ...
    two meet is `ConsumeCharAction`, which takes the character its gate found, so a way built from that pair takes the
    set the gate admits.
...
def spelling_size(grammar, node, seen=()):
    ...
    name. `references` gives the productions the subtree names — which no walk of the fields could find, a `RefCall`'s
    name and an `(emit)`'s code both being strings — so asking it once at the top reaches every name and no node. Asking
    both at every level would count a named production once per level standing above it.
    """
