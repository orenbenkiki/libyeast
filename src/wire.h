...
// ys_write_token(). The arm owns the storage that holds a token's text once the read unescapes it. The arm is state
// and not a function.
typedef struct ys_wire {
    ys_memory memory;
    ys_source source;     // the wire's bytes, and the buffer they land in.
    size_t consumed;      // the count of those bytes the lines handed back have taken.
    size_t scanned;       // the reach of the search for the break of the current line. It starts at `consumed`.
    ...
    // a resource failure ys_next_line hit. `0` for none. YS_FAILED_STREAM names the reader and YS_FAILED_MEMORY names
    // the allocator.
    int fault;
...
// write landed. YS_FAILED_STREAM with `errno` the writer's value. YS_FAILED_ACTION with `errno` EINVAL where the token
// has no wire form.
