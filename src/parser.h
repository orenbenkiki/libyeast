...
// A single automaton parses. Characters drive the automaton. It emits yeast tokens into a queue. `ys_parser` holds
// the whole execution state. That state is a window over the input and a stack of the productions the parser is
// inside. The state also holds a queue of the tokens the parser has built. The automaton's own state sits there too.
//
// The C call stack holds none of that state. `ys_parser_read` can therefore hand back a token from the middle of a
// production and resume there on the next call.
//
// The parser has a single layer. A scanner would need a vocabulary between itself and the parser, and yeast writes
// none. The automaton's output already is the token stream. That stream is the begin and end markers and the
// classified spans of input.
...
// `parser_tables.h`. This is only their type. The automaton begins in the state `YS_STATE_START` names. That state sits
// in this file rather than in the generated tables.
...
//
// A string parser's window is the caller's buffer. The parser does not copy, grow or free that buffer. The window holds
// the whole input from its first byte. `base` stays `0`, and the source has no reader to give the window more.
//
...
// Move past the characters a consume took. Such a consume holds no line break, and `generator/check_decoder.py` holds
// the grammar to that.
...
// parser works a token's text out from the token's marks and the window. The parser does that at the moment it hands
// the token back. The text is valid then and no later. An error's text is not in the input at all, and is the parser's
// message.
...
// ahead of the run.
//
// The undecided tokens form a suffix. A pair of runs make such a suffix. The empty lines that
// open a block scalar are the first run. The line of an implicit key is the second run. A run waits on input the parser
// has still to read.
//
// The parser resolves a run by injecting a marker ahead of it. The marker
// is `end-scalar` for a block scalar whose empty lines the parser chomped away. It is `begin-mapping` for a line that
// the parse reads as a key. A single run is open at a time.
//
// The injected marker is `ahead`. `ahead` is a token of its own rather than a token `tokens` holds. The queue therefore
// needs no room for it. The marker precedes what the queue holds. The automaton runs only while the queue's first token
// belongs to the open run. A decided token ahead of the run has already gone back to the caller.
//
// Injecting a marker therefore cannot fail for want of memory.
typedef struct ys_queue {
    ...
    bool is_run_open; // the next token the parser emits joins the open run.
...
// Put a decided token ahead of an open run, and ahead of the rest of the queue.
...
// Enter a production with parameter `n`. The parser resumes at `return_state` when the production matches. `YS_OK`, or
// `YS_FAILED_MEMORY` where the cap or the allocator refused the frame.
...
// there.
//
// The failure path therefore allocates nothing. A parser that has run out of memory cannot allocate anyway. The
// failure path frees nothing. A token's text comes from the input or from a
// static string. The error code does not change what `ys_are_tokens_stable` answers.
//
// The message does not say what the parser found. The first `YS_CODE_UNPARSED_TEXT` token behind
// the error begins at exactly the byte that failed.
//
// The error token is a token of its own rather than a token the queue holds. It follows
...
// A `ys_parser` is the parsing arm of a `ys_token_source`. The parser holds the whole execution state of the automaton.
// The description at the top of this file says what that state allows.
typedef struct ys_parser {
    ys_memory memory; // the allocator the parser may use, and the cap on the memory it may allocate.
    ys_window window; // the readable bytes and where the next character sits among them.
    ys_queue queue;   // the tokens built and not yet handed back. The undecided ones sit among them.
    ...
    // A resource failure `ys_read_token` is to report. The field holds `0` for none. It holds `YS_FAILED_STREAM` where
    // the reader failed, and `YS_FAILED_MEMORY` where the allocator did. The field holds the name from `ys_status`
    // rather than its number.
    int fault;
    // the source has nothing left, and answers `YS_FAILED_ACTION` from here on. The stream ended, or the source
    // faulted.
    bool is_done;
} ys_parser;

// Initialize a freshly-allocated parser arm. `memory` bounds the allocation, and `options` gives the resume policy.
...
// Read the next token into `token`. `YS_OK` with it filled. `YS_FAILED_STREAM` where the reader failed.
// `YS_FAILED_MEMORY` where the allocator did. `errno` holds the callback's value. `YS_FAILED_ACTION` with `errno`
// ENODATA once a caller reads past the end of the stream.
...
//
// `YS_OK`, or the fault a failed fill sets in `ys_parser::fault` before ending the source. That is `YS_FAILED_STREAM`
// where the source's reader failed, and `YS_FAILED_MEMORY` where the cap or the allocator did.
int ys_parser_fill(ys_parser *parser, size_t wanted);

// Hand back a malformed document. That is the error token `ys_parser_error` describes. Its text is `message`.
// `ys_error::resume` says what the parser does with the rest of the input.
//
