...
// A `ys_token_sink` takes an arm. The kind says how `ys_write_token` renders a token. Both arms hold a byte transport.
// The sink is a tagged struct rather than a union. The wire writer writes a token in the wire format. The YAML emitter
// writes the bytes a token spans.
...
// holds no `ys_memory` to track a cap against. The sink lives on the heap. A caller holds the sink opaquely.
struct ys_token_sink {
    ys_sink_kind kind;
    ys_allocator allocator; // the allocator that made the sink.
    ys_bytes_writer writer; // the writer the tokens go to. The kind names that writer.
};
