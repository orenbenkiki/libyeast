...
// Generated by `generator/grammar2decoder.py` from `grammar/yeast-spec-1.2.yaml`. Do not edit.
// clang-format off
...
// A character's key holds the id of the character the grammar names in bits 0..5,
// a bit per character set the grammar tests in bits 6..25, and the bytes the
// character consumed in bits 26..28. Use the macros below.
#define YS_LEN(key)            (((key) >> 26) & 0x7u)     // the bytes a character consumed
#define YS_LENGTH_BITS(length) ((uint32_t)(length) << 26) // the way the decoder writes them

// The key of a character the grammar names. A key stays put. The sets and the length stay put
// as well. Testing for a character is one comparison.
...
// One bit per character set the grammar tests. The generator evaluated the unions and subtractions already.
...
// The bit of a character set. Its id finds the bit.
...
#define YS_KEY_CONTROL    0x04008000u // a C0 control. that character belongs to no set.
...
// Such a character is a 2-byte, 3-byte or 4-byte sequence. `decoder.c` ORs its length in.
